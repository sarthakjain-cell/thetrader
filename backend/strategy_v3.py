...
class StrategyV3:
    ...
    def backtest(self, initial_capital=100000.0, risk_per_trade=0.01):
        ...
        for i in range(1, len(self.df)):
            ...
            if position == 0:
                vol_condition = row['Volume'] > (1.2 * row['Vol_SMA_20'])
                if (row['EMA_Fast'] > row['EMA_Slow']) and (row['RSI'] > self.p['rsi_thresh']) and (row['ADX'] > self.p['adx_thresh']) and vol_condition:
                    
                    exec_price = price * (1 + slippage)
                    atr = row['ATR']
                    
                    initial_stop_dist = atr * 3.0 # Hardcoded 3.0x ATR safety net
