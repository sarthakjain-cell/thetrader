...
class StrategyV2:
    def __init__(self, df, params):
        ...
        # Drop NaNs generated by windows (like 50-day EMA)
    ...
    def backtest(self, initial_capital=100000.0, risk_per_trade=0.01):
        ...
        for i in range(1, len(self.df)):
            ...
            if position > 0:
                ...
                # B. Trend-Following Exit (Fast crosses under Slow)
            ...
            if position == 0:
                # Core Entry Logic: EMA + RSI + ADX Trend Filter
                if (row['EMA_Fast'] > row['EMA_Slow']) and (row['RSI'] > self.p['rsi_thresh']) and (row['ADX'] > self.p['adx_thresh']):
                    
                    exec_price = price * (1 + slippage)
                    atr = row['ATR']
                    
                    # Risk Management: Sizing based on ATR Stop Distance
