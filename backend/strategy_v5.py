...
class StrategyV5:
    ...
    def backtest(self, initial_capital=100000.0, risk_per_trade=0.01):
        ...
        for i in range(1, len(self.df)):
            ...
            if position == 0:
                # Engine Entry relies on Pre-computed `Entry_Allowed` (Rotation) AND `Market_Regime_Up` (Nifty > 200 SMA)
                if row.get('Entry_Allowed', False) and row.get('Market_Regime_Up', False) and (row['EMA_Fast'] > row['EMA_Slow']) and (row['RSI'] > self.p['rsi_thresh']) and (row['ADX'] > self.p['adx_thresh']):
