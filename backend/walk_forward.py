...
def apply_strategy(df, fast_ema, slow_ema, rsi_thresh, adx_thresh=20):
    ...
    temp_df['ADX'] = ta.trend.adx(temp_df['High'], temp_df['Low'], temp_df['Close'], window=14)
    
    temp_df['Signal'] = 0
    # Buy Condition: Trend up + Momentum + Strong Trend
    buy_cond = (temp_df['EMA_Fast'] > temp_df['EMA_Slow']) & (temp_df['RSI'] > rsi_thresh) & (temp_df['ADX'] > adx_thresh)
    # Sell Condition (Momentum loss or trend breakdown)
...
def walk_forward_optimization():
    ...
    combinations = list(itertools.product(fast_emas, slow_emas, rsi_thresholds, adx_thresholds))
    ...
    for fast_ema, slow_ema, rsi_thresh, adx_thresh in combinations:
        ...
        for train_df, test_df, window_name in windows:
            ...
            train_signals = apply_strategy(train_df, fast_ema, slow_ema, rsi_thresh, adx_thresh)
            train_results = run_backtest_window(train_signals)
            
            # If it fails in training, discard immediately to save time (we want Profit Factor > 1.05 at least)
            if train_results["profit_factor"] < 1.05 or train_results["total_trades"] < 5:
                valid_across_windows = False
                break
                
            # 2. Test (Does it survive unseen out-of-sample data?)
            test_signals = apply_strategy(test_df, fast_ema, slow_ema, rsi_thresh, adx_thresh)
        ...
        if valid_across_windows:
            ...
            if avg_test_pf > best_overall_pf:
                ...
                best_overall_params = (fast_ema, slow_ema, rsi_thresh, adx_thresh)
                
    if best_overall_params:
        log.info(f"OPTIMIZATION COMPLETE! Found Mathematical Edge.")
        log.info(f"Winning Parameters: Fast EMA {best_overall_params[0]}, Slow EMA {best_overall_params[1]}, RSI > {best_overall_params[2]}, ADX > {best_overall_params[3]}")
